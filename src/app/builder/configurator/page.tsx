...
const isPartCompatible = (part: Part, currentSelectedCar: string): boolean => {
    ...
    if (!partDesignator || partDesignator === "vw mqb") {
        return true;
    }
...
type SortablePartKey = 'name' | 'price';
...
const AutomotiveBuildConfigurator = () => {
    ...
    const [selectedTurbo, setSelectedTurbo] = useState<string>(TURBO_OPTIONS[0]);
    const [selectedTransmission, setSelectedTransmission] = useState<string>(TRANSMISSION_OPTIONS[0]);

    const [sortConfig, setSortConfig] = useState<{ key: SortablePartKey; direction: 'ascending' | 'descending' }>({
        key: 'name',
        direction: 'ascending',
    });
                ...
                querySnapshot.docs.forEach(doc => {
                    ...
                    if (
                        ...
                        allParts.push({
                            ...
                            url: data.url,
                            compatible_turbos: Array.isArray(data.compatible_turbos) ? data.compatible_turbos.map(String) : undefined,
                            compatible_transmissions: Array.isArray(data.compatible_transmissions) ? data.compatible_transmissions.map(String) : undefined,
                        });
                    } else {
    ...
    const categorizedParts: DisplayCategory[] = useMemo(() => {
        ...
        const compatibleParts = firestoreParts.filter(part => isPartCompatible(part, selectedCar));
    ...
    const selectedPartsByCategory: DisplayCategory[] = useMemo(() => {
        ...
        Object.values(selectedParts).forEach(part => {
            ...
            if (categoryData) {
                ...
                const miscCategory = categoriesMap.get('miscellaneous');
                if (miscCategory) miscCategory.parts.push(part);
                else console.warn(`Selected part (ID: ${part.id}) has unknown category "${part.category}" and 'miscellaneous' not defined.`);
            }
        });
    ...
    const filterTunes = (parts: Part[]): Part[] => {
        return parts.filter(part => {
            const turboMatch = selectedTurbo === 'All' ||
                ...
                (!part.compatible_turbos && part.name.toLowerCase().includes(selectedTurbo.toLowerCase()));

            const transmissionMatch = selectedTransmission === 'All' ||
                (part.compatible_transmissions?.includes(selectedTransmission)) ||
                (!part.compatible_transmissions && part.name.toLowerCase().includes(selectedTransmission.toLowerCase()));

            return turboMatch && transmissionMatch;
        });
    };

    const requestSort = (key: SortablePartKey) => {
    ...
    return (
        <SidebarProvider>
                ...
                <header className="flex h-16 shrink-0 items-center gap-2 border-b px-4 bg-background sticky top-0 z-50">
                    ...
                    <div className="flex flex-1 items-center justify-end gap-4 p-4">
                        <Select value={selectedCar} onValueChange={(value) => {
                            ...
                            <SelectTrigger className="w-auto min-w-[200px] sm:w-[220px] bg-background text-foreground">
                    ...
                    <div className="container mx-auto p-4 md:p-6 lg:p-8">
                        ...
                        {loadingParts ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6 lg:gap-8">
                                <div className="category-selection md:col-span-3 flex flex-col space-y-2 md:space-y-3">
                                    {categorizedParts.map((category) => (
                                        <Button
                                            key={category.value}
                                            variant="ghost"
                                            className={cn(
                                                "w-full justify-start px-4 py-3 rounded-md text-foreground bg-card hover:bg-accent hover:text-accent-foreground transition duration-150 ease-in-out font-medium text-sm md:text-base",
                                                activeCategory === category.value && "bg-primary text-primary-foreground ring-2 ring-primary-focus"
                                            )}
                                ...
                                <div className="part-selection md:col-span-6 bg-card p-3 rounded-md border overflow-y-auto max-h-[calc(100vh-12rem)]">
                                    {activeCategory ? (
                                        categorizedParts
                                            .filter(cat => cat.value === activeCategory)
                                            .map((category) => {
                                                let partsToDisplay = category.value === 'tunes'
                                                    ? filterTunes(category.parts)
                                                    : category.parts;

                                                if (sortConfig.key) {
                                                    partsToDisplay = [...partsToDisplay].sort((a, b) => {
                                                ...
                                                return (
                                                    <div key={category.value}>
                                                        <h2 className="text-2xl font-semibold text-foreground mb-0 sticky top-0 bg-card py-3 z-20 border-b">
                                                            {category.name}
                                                        </h2>

                                                        <div className="sticky top-[calc(2rem+1.5rem)] bg-card pt-2 pb-3 z-10 border-b mb-3">
                                                            ...
                                                            <div className="flex flex-wrap gap-2 items-center">
                                                                ...
                                                                <Button
                                                                    variant={sortConfig.key === 'name' ? "customblue" : "outline"}
                                                                    size="sm"
                                                                ...
                                                                <Button
                                                                    variant={sortConfig.key === 'price' ? "customblue" : "outline"}
                                                                    size="sm"
                                ...
                                <div className="selected-parts md:col-span-3 bg-blue-700/90 backdrop-blur-md rounded-md shadow-xl p-4 md:p-6 space-y-4 text-white max-h-[calc(100vh-12rem)] overflow-y-auto">
                                    <h2 className="text-xl font-semibold mb-4 sticky top-0 bg-blue-700/90 py-2 z-10">
                                        Your Build
                                    </h2>
                                    <ul id="selected-parts-list" className="space-y-3">
                                        {Object.keys(selectedParts).length === 0 ? (
                                            ...
                                            Object.entries(selectedParts).map(([partId, part]) => (
                                                <li
                                                    key={partId}
                                                    className="flex justify-between items-center text-sm bg-blue-600/60 p-2 rounded-md"
                                                >
                                    ...
                                    <div className="summary-section border-t border-blue-500 pt-4 space-y-3 sticky bottom-0 bg-blue-700/90 py-3">
            ...
            <Dialog open={isBuyNowPopupOpen} onOpenChange={setIsBuyNowPopupOpen}>
                <DialogContent className="sm:max-w-[1200px] max-h-[80vh] overflow-y-auto bg-card text-foreground">
                    <DialogHeader>
                        <DialogTitle className="text-xl">Confirm Your Build</DialogTitle>
                        <DialogDescription>
                            Review the parts in your build. Clicking &quot;Proceed to Checkout&quot; will open each part&apos;s URL in a new tab.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <DialogFooter className="flex flex-col sm:flex-row sm:justify-between items-center gap-4 pt-4 border-t border-border">
